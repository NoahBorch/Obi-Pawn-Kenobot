...
class TestSearch(unittest.TestCase):
    ...
    def test_mate_in_one(self):
        ...
        self.assertGreaterEqual(eval_score, CHECKMATE_BASE_SCORE)  # Adjust if you use a different MATE_SCORE
...
def test_bratko_kopec_positions(self):
    ...
    # Use new logger config that returns the log folder path
    log_path = configure_logging("debug", save_to_file=True, logdir=f"../../.logs/search_test/depth{depth}/Bratko-Kopec_split")
    set_debug_config_for_module("search", True)
    set_debug_config_for_module("play", True)

    logger.debug(f"Using depth: {depth}")
    epd_path = Path(__file__).parent / "Bratko-Kopec_test_suite.txt"
    ...
    try:
        for epd_line in epd_positions:
            ...
            if expected_moves:
                results_log.append({
                "expected_move": expected_move.uci() })
            results_log.append({
                "best_move": best_move.uci() if best_move else None,
                "eval": eval_score,
                "result": result_status
